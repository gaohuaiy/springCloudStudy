    ...
    <div class="row">
      <div class="col-md-6">
        <p>
          ...
          &nbsp;
          <button v-on:click="all(1)" class="btn btn-white btn-default btn-round">
  ...
  export default {
    ...
    methods: {
      ...
      all() {
        let _this = this;
        Loading.show();
        _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/category/all', )
                .then((response)=>{
          Loading.hide();
      ...
      save() {
        ...
        Loading.show();
        _this.$axios.post(process.env.VUE_APP_SERVER + '/business/admin/category/save', _this.category).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
      ...
      del(id) {
        ...
        Confirm.show("删除分类后不可恢复，确认删除？", function () {
          Loading.show();
          _this.$axios.delete(process.env.VUE_APP_SERVER + '/business/admin/category/delete/' + id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
              _this.all();
